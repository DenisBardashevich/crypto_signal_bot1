...
def check_signals(df, symbol):
    try:
        ...
        # Новый фильтр по объёму на 15м: не брать сигнал, если объём ниже 0.5 среднего за 20 свечей
        if last['volume'] < df['volume'].rolling(20).mean().iloc[-1] * 0.5:
            logging.info(f"{symbol}: объём ниже 0.5 среднего за 20 свечей, сигнал не формируется")
            return []
